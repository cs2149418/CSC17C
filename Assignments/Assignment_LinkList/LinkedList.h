 ...
 * Created on October 10, 2017, 3:22 PM
...
class LinkedList{
    ...
    public:
        ...
        void insrtBeg(const T&);
        void insrtBefore(T,T);
        void insrtAfter(T,T);
        void deleteVal(T);
};
...
LinkedList<T>::LinkedList(const T &data){
    ...
    front->linkPtr=nullptr; //At the moment not pointing it to anything
}
...
void LinkedList<T>::prntLst(){  
    ...
    }while(next!=nullptr);         //Loop until reaching the end
...
void LinkedList<T>::endLst(){
    ...
    }while(temp!=nullptr);     //Your done when you hit the end
}
...
void LinkedList<T>::addLst(const T &data){
    ...
    add->linkPtr=nullptr;         //Set the pointer to NULL
...
int LinkedList<T>::findLst(const T &value){
    ...
    }while(temp!=nullptr);     //End when reaching the end of the linked list
...
void LinkedList<T>::fndLst(const T &value){
    ...
    }while(found!=nullptr);                     //End of the list
...
int LinkedList<T>::cntLst(){
    ...
    do{
        n++;            //Increment the counter
        temp=temp->linkPtr;//Move to the next link
    }while(temp!=nullptr); //Loop until the end is reached
...
template<class T>
void LinkedList<T>::insrtBeg(const T& num)
{
    Link* newNode;
    newNode=new Link;
    newNode->data=num;
...
template<class T>
void LinkedList<T>::insrtBefore(T link,T num)
{
    Link *newNode;
    Link *nodePtr;
    Link *prev=nullptr;
    ...
    {
        front=newNode;
        newNode->linkPtr=nullptr;
    }
    else
    {
        nodePtr=front;
        while(nodePtr!=nullptr&&nodePtr->data<link)
        {
            prev=nodePtr;
        ...
        else
        {
            ...
            newNode->linkPtr=nodePtr;
        }
    }
}

template<class T>
void LinkedList<T>::insrtAfter(T link,T num)
{
    Link *newNode;
    Link *nodePtr;
    Link *prev=nullptr;
